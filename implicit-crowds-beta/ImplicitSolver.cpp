...
void ImplicitSolver::createAgent(Vector2D pos, Vector2D goal, Vector2D velocity, double radius, double prefSpeed, double maxSpeed, double goalRadius, int gid)
{
	...
	if (newAgent != NULL) {
		newAgent->init(pos, goal, velocity, radius, prefSpeed, maxSpeed, goalRadius, gid, _noAgents, _spatialDatabase);
...
void ImplicitSolver::updateSimulation()
{
	...
	MGlobal::displayInfo("update 1");

	if (_reachedGoals) return;
	MGlobal::displayInfo("update 2");
	this->initializeProblem();
	MGlobal::displayInfo("update 3");
	this->minimize(_vNew);
	MGlobal::displayInfo("update 4");
	this->finalizeProblem();
	MGlobal::displayInfo("update 5");
	...
	MGlobal::displayInfo("update 6");

	_globalTime += _dt;
	_iteration++;
	MGlobal::displayInfo("update END");
}
