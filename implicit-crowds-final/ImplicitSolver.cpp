...
void ImplicitSolver::init(double x, double y)
{
	...
	_iteration = 0;
	_globalTime = 0;
	_spatialDatabase = new SpatialProximityDatabase(VectorXd::Zero(2, 1), Vector2D(x, y), Vector2D(10, 10));
	_k = 1.5;
	_p = 2.;
	_t0 = 3.;
	_ksi = 2.;
...
void ImplicitSolver::createAgent(Vector2D pos, Vector2D goal, Vector2D velocity, double radius, double prefSpeed, double maxSpeed, double goalRadius, int gid, int bool_agent)
{
	if (bool_agent == 1) {
		Agent* newAgent = new Agent();
		if (newAgent != NULL) {
			newAgent->init(pos, goal, velocity, radius, prefSpeed, maxSpeed, goalRadius, gid, _noAgents, _spatialDatabase, false);
			cmd_agent.push_back(newAgent);
			_agents.push_back(newAgent);
			++_noAgents;
		}
	}
	else {
		Agent* newObstacle = new Agent();
		if (newObstacle != NULL) {
			newObstacle->init(pos, goal, velocity, radius, prefSpeed, maxSpeed, goalRadius, gid, _noAgents, _spatialDatabase, true);
			_obstacles.push_back(newObstacle);
			_agents.push_back(newObstacle);
			++_noAgents;
		}
	}
}
...
void ImplicitSolver::setParameter(string key, string value)
{
	if (key == "k") {
		...
		setDouble(value, _ksi);
	}
	else if (key == "eps") {
		setDouble(value, _eps);
	}
	else if (key == "repulsive") {
		setDouble(value, _eta);
	}
	else if (key == "neighborDist") {
		setDouble(value, _neighborDist);
	}
	else if (key == "newtonIter") {
		setInt(value, _newtonIter);
	}
	else if (key == "lbfgsWindow") {
		setInt(value, _window);
	}
	else if (key == "eps_x") {
		setDouble(value, _eps_x);
	}
}
...
void ImplicitSolver::initializeProblem()
{
...
void ImplicitSolver::finalizeProblem()
{
	for (unsigned int i = 0; i < _noAgents; ++i)
	{
		if (_agents[i]->enabled())
...
double ImplicitSolver::value(const VectorXd &vNew)
{
...
#pragma omp parallel for shared(exit) reduction(+:f) num_threads(_max_threads)
	for (int i = 0; i < _activeAgents; ++i)
	{
		if (!exit)
		{
			...
			{
				const Agent* other = static_cast<Agent*>(_nn[i][j]);
...
double ImplicitSolver::value(const VectorXd &vNew, VectorXd &grad)
{
	_posNew = _pos + vNew*_dt;
	// acceleration and goal velocity contributions
...
bool ImplicitSolver::min_distance_energy(double Pa_x, double Pa_y, double Pb_x, double Pb_y, double Va_x, double Va_y, double Vb_x, double Vb_y, double radius, double& energy, double* grad)
{
	energy = 0;
	double Xx = Pb_x - Pa_x;
	double Xy = Pb_y - Pa_y;
...
double ImplicitSolver::inverse_ttc_energy(double Pa_x, double Pa_y, double Pb_x, double Pb_y, double Va_x, double Va_y, double Vb_x, double Vb_y, double radius, double* grad)
{
...
double ImplicitSolver::linesearch(const Vector<double> & x0, const Vector<double> & searchDir, const double phi0, const Vector<double>& grad, const double alpha_init)
{
...
void ImplicitSolver::minimize(Vector<double> & x0)
{
